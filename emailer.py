...
def build_email_html(jobs: list) -> str:
    ...
    mid = [j for j in jobs if 6.0 <= j["score"] < 7.5]
      ...
      <div style="max-width:680px; margin:0 auto; padding:32px 16px;">
        ...
        <div style="display:flex; gap:12px; margin-bottom:28px; justify-content:center;">
          <div style="background:#1e293b; border-radius:10px; padding:14px 24px; text-align:center;">
            ...
            <p style="margin:4px 0 0 0; font-size:12px; color:#94a3b8;">Ofertas top (≥7.5)</p>
          </div>
          <div style="background:#1e293b; border-radius:10px; padding:14px 24px; text-align:center;">
            <p style="margin:0; font-size:28px; font-weight:800; color:#f59e0b;">{len(mid)}</p>
            <p style="margin:4px 0 0 0; font-size:12px; color:#94a3b8;">Interesantes (6–7.5)</p>
          </div>
          <div style="background:#1e293b; border-radius:10px; padding:14px 24px; text-align:center;">
            <p style="margin:0; font-size:28px; font-weight:800; color:#6366f1;">{len(jobs)}</p>
            <p style="margin:4px 0 0 0; font-size:12px; color:#94a3b8;">Total filtradas</p>
          </div>
        </div>
        ...
        <div style="margin-top:32px; text-align:center; color:#334155; font-size:12px;">
          ...
          <p>Fuentes: Remote OK · We Work Remotely · Remotive</p>
        </div>
...
def send_email(jobs: list):
    ...
    msg["Subject"] = f"🎯 {top_count} oferta{'s' if top_count != 1 else ''} top hoy · Job Hunt {today}"
    msg["From"] = gmail_user
    msg["To"] = recipient
